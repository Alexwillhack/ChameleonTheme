<?php
/**
 * qweTheme functions and definitions
 *
 ...
 */
function qwetheme_setup() {
	/*
		* Make theme available for translation.
		* Translations can be filed in the /languages/ directory.
		* If you're building a theme based on qweTheme, use a find and replace
		* to change 'qwetheme' to the name of your theme in all the template files.
		*/
	load_theme_textdomain( 'qwetheme', get_template_directory() . '/languages' );
	...
	add_theme_support(
		'custom-background',
		apply_filters(
			'qwetheme_custom_background_args',
			array(
...
add_action( 'after_setup_theme', 'qwetheme_setup' );
 ...
 */
function qwetheme_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'qwetheme_content_width', 640 );
}
add_action( 'after_setup_theme', 'qwetheme_content_width', 0 );
 ...
 */
function qwetheme_widgets_init() {
	register_sidebar(
		array(
			'name'          => esc_html__( 'Sidebar', 'qwetheme' ),
			'id'            => 'sidebar-1',
			'description'   => esc_html__( 'Add widgets here.', 'qwetheme' ),
...
add_action( 'widgets_init', 'qwetheme_widgets_init' );
